...
const char* String::Find(const char* findstring, const char* replacestring)
{
	int* matchIndexes;
	char* replacedString = new char[strlen(m_string) + 1];
	strcpy_s(replacedString, strlen(m_string) + 1, m_string);
	matchIndexes = new int[1];
	int numberOfMatches = 0;
	for (int i = 0; i < strlen(m_string); i++)
	{
	...
	for (int i = 0; i < numberOfMatches; i++)
	{
		int replaceIndex = matchIndexes[i];
		replacedString[replaceIndex] = *replacestring;
	}
...
const char* String::ReadFromConsole()
{
	...
	std::cin >> temp;
	char* readInput = new char[strlen(temp) + 1];
